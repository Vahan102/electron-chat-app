...
const pareatElement = document.querySelector("ul");
...
export function createGroup(link, name, type, imge) {
...
;
getRequest("getUserServers", token, {}).then((res) => {
    for (let i = 0; i < res.data.length; i++) {
        ...
        document.addEventListener("keydown", (event) => {
            if (isHover && event.key === "Enter") {
                div.remove();
                sendRequest("deleteServer", "DELETE", "deleteServer", token, { link: res.data[i].link }).then((res) => {
                    console.log(res);
                });
            }
        });
...
function clickTheServerIcon(link, name, img) {
    return __awaiter(this, void 0, void 0, function* () {
...
function createMessege(box, value, src) {
    const div = document.createElement("div");
    const img = document.createElement("img");
    const body = document.createElement("div");
    const p = document.createElement("p");
